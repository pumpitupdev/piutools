...
void ProcessRequest(MDGRequest * req, MDGResponse * res, unsigned int print_info){
    ...
    switch(req->operation_code){
        ...
        case OP_CONVERT_HASH:
            ...
            if(!resp_found){
                printf("[%s:%s] WARNING: RESPONSE NOT FOUND FOR REQUEST: %s\n",__FILE__,__FUNCTION__,hex_to_str(req->dog_data,req->dog_bytes));
            }
            break;
...
void MicroDog_Init(const char* path_to_dongle_file){
    DEBUG_PRINT("[IO.MicroDog] Starting Emulator...\n");
    load_config(path_to_dongle_file);
}