...
const unsigned char KEY_HDD_MAGICWORD[13] = {0x6B, 0x5F, 0x12, 0x51, 0x25, 0x73, 0x77, 0xD2, 0x9F, 0x8A, 0xF1, 0xDA, 0x53 };
...
static void create_hdd_file(void){
    int key_offset = 0x7000;

    int ata_offset = 0x7040;
    FILE* fp = fopen(fake_hdd_file_path,"wb");
    
    fseek(fp,key_offset,0);
    fwrite(KEY_HDD_MAGICWORD,sizeof(KEY_HDD_MAGICWORD),1,fp);
    fseek(fp,ata_offset,0);
    // Write ATA INFO
